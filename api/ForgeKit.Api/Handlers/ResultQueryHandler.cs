...
/// <summary>
/// Abstract base class for query handlers returning Result{TResponse}.
/// 
...
/// 
/// This replaces IQueryHandler which is no longer needed with Result Pattern.
/// </summary>
/// <typeparam name="TRequest">The query request type</typeparam>
/// <typeparam name="TResponse">The response data type</typeparam>
public abstract class ResultQueryHandler<TRequest, TResponse>(ILogger<ResultQueryHandler<TRequest, TResponse>> logger) : IRequestHandler<TRequest, Result<TResponse>>
    where TRequest : IRequest<Result<TResponse>>
{
    ...
    /// </summary>
    protected ILogger<ResultQueryHandler<TRequest, TResponse>> Logger { get; } = logger ?? throw new ArgumentNullException(nameof(logger));

    /// <summary>
    /// Main handler method. Derived classes override this to implement query logic.
    /// </summary>
