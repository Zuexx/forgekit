// No "server-only" guard here on purpose -- this file exists so lib/db/sqlite.ts (the guarded
// wrapper every app import should use instead) and the CLI-facing lib/auth-instance.ts (which
// the better-auth CLI cannot load through a "server-only" import at all) can both reach the
// same underlying connection without either one re-triggering the other's guard. Never import
// this file directly from app code -- import "@/lib/db/sqlite" instead, which re-exports
// everything here behind the real guard.
