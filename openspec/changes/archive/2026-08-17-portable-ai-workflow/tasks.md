...
- [x] 1.1 Declare the three workflow CLIs in a new root `package.json` and commit the lockfile.
...
- [x] 1.2 Confirm the root install does not disturb the frontend. Done when `app/`'s lockfile is
  unchanged by `git status`, and `scripts/verify.sh` still exits 0.

- [x] 1.3 Make `scripts/verify.sh` resolve the declared binaries before global ones. It currently
...
- [x] 2.1 Declare the CodeGraph MCP server in a new root `.mcp.json` pointing at the local
...
- [x] 2.2 Declare the Superpowers plugin in a new tracked `.claude/settings.json`. Done when the
...
- [x] 3.1 Implement `scripts/preflight.sh` covering the four checks the spec requires — declared
  tools present, workflow config readable, code index present and reflecting current source, git
  ...
  *Amended during implementation: staleness was first written as "not older than `HEAD`", which
  turned the check red after every commit even though committing modifies no source file. It
  compares against the newest tracked source file instead. See design.md.*

- [x] 3.2 Add the cited-capability resolution check, resolving `superpowers:*` by glob against
...
- [x] 3.3 Verify the config-readback check catches a silently empty section. Done when
...
- [x] 3.4 Make the repository's scripts runnable where file modes are not preserved, and report
...
- [x] 3.5 Stop `.githooks/pre-push` reading past the `## OpenSpec Coverage` section. It ends the
...
- [x] 4.1 Correct grillme in `AGENTS.md`: remove the `grilling` row from the overlapping-skills
...
- [x] 5.1 Record the workflow CLI version policy in `docs/DEPENDENCY_CONSTRAINTS.md`, following
...
- [x] 5.2 Remove the `.mem0-src/` entries from `.gitignore` and
...
- [x] 6.1 Verify a generated product receives a working toolchain. Done when
  `dotnet new forgekit` output contains `package.json`, `.mcp.json`, `.claude/settings.json`,
  and `scripts/preflight.sh`, and `pnpm install && pnpm preflight` in that generated project
  exits 0 after its hooks are enabled and indexed.

  *Amended during implementation: written as `./scripts/preflight.sh`, which cannot run in a
  generated product — `dotnet new` does not carry the executable bit. That is task 3.4.*
