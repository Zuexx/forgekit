# Architecture Decision Records (ADRs) Specification
...
- Each ADR SHALL focus on generic pattern (not Scaffold-specific)
...
- Each ADR SHALL provide Scaffold implementation example
...
- **AND** it includes Scaffold IModule example
...
- **AND** it shows Scaffold 30-day restore example
...
- **AND** it shows Scaffold ValidationBehavior example
...
- **AND** it shows Scaffold direct DbContext approach
...
- **AND** it shows Scaffold IAuditContext example
...
- **AND** it shows Scaffold ErrorResponse with codes
...
- **AND** it shows Scaffold IUnitOfWork example
...
**Acceptance Criteria:**
- Each ADR SHALL be readable by someone unfamiliar with Scaffold
- Scaffold SHALL be presented as example, not the subject
...
- **WHEN** a team outside Scaffold reads an ADR
- **THEN** they understand the pattern without Scaffold knowledge
- **AND** they can apply pattern to their own project
- **AND** Scaffold example helps clarify but isn't required to understand
